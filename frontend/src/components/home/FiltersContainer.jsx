...
export default function FiltersContainer({ applyFilters }) {
  ...
  const [filters, setFilters] = useState({
    is_active: searchParams.get('is_active') === 'true' ? true : searchParams.get('is_active') === 'false' ? false : null,
  ...
  const getOrderName = () => {
    if (filters.order_by === 'created_at' && filters.order_dir === 'asc') return 'Раньше создано';
  ...
  const sortOptions = [
    ...
    { label: 'Раньше создано', value: { order_by: 'created_at', order_dir: 'asc' } },
    { label: 'Меньше кликов', value: { order_by: 'clicks', order_dir: 'asc' } },
    { label: 'Больше кликов', value: { order_by: 'clicks', order_dir: 'desc' } },
  ];
            ...
            <DropdownCard
                trigger={
                  <button type="button" className="filter-pill-btn">
                    ...
                    <span>{filters.is_active === null ? 'Все' : filters.is_active ? 'Активные' : 'Неактивные'}</span>
                  </button>
                }
            >
              <div className="dropdown-options-box">
                <TripleStateCheckbox
                    ...
                    setFilter={(value) => setFilters((prev) => ({ ...prev, is_active: value }))}
                />
              </div>
            </DropdownCard>
                ...
                {sortOptions.map((option) => (
                    <button
                        ...
                        className={`dropdown-option-item close-dropdown ${
                            option.value.order_by === filters.order_by && option.value.order_dir === filters.order_dir ? 'is-active' : ''
                        }`}
                        onClick={() => setFilters((prev) => ({ ...prev, ...option.value }))}
                    >
                      {option.label}
                    </button>
                ))}
